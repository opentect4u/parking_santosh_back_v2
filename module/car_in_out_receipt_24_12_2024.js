...
const insert_receipt = (userData, receipt_no, base_amt, cgst, sgst, paid_amt, gst_flag, trans_flag, paymode) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            gst_value = parseFloat(cgst) + parseFloat(sgst);
            console.log(gst_value,'gst_value');
            
            base_amt = parseFloat(paid_amt) - parseFloat(gst_value)
            console.log(base_amt);
            

            let receipt_fields = `(receipt_no, user_id, base_amt, cgst, sgst, paid_amt, gst_flag, trans_flag, pay_mode, created_at)`,
                receipt_values = `(${receipt_no},${userData.id},${base_amt},${cgst}, ${sgst}, ${paid_amt}, '${gst_flag}', '${trans_flag}', '${paymode}', '${datetime}')`;
