...
const edit_device = async (req, res) => {
  ...
  // console.log(data);
  var custId = req.session.user.user_data.customer_id;
  let select = "a.app_id,a.customer_id,a.device_type,a.dev_mod,a.report_flag,a.total_collection,a.adv_pay,a.adv_value,a.grace_period_flag,a.grace_value,a.redirection_flag,a.gst_flag,a.pay_mode_flag,b.customer_id,b.customer_name,b.dev_mod",
    table_name = "md_setting a, md_customer b",
    whr = `a.customer_id = b.customer_id AND a.customer_id=${custId} AND a.app_id='${data.dev_id}'`;
...
const save_edit_device = async (req, res) => {
  try {
    ...
    let fields = `device_type='${value.dev_type}',report_flag='${
      ...
      },redirection_flag='${ value.redirect_flag == "Y" ? "Y" : "N"}',gst_flag='${ value.gst_flag == "Y" ? "Y" : "N"}',pay_mode_flag='${ value.pay_mode_flag == "Y" ? "Y" : "N"}',modified_by='${custId}',updated_at='${datetime}'`,
...
const save_add_device = async (req, res) => {
  try {
    ...
    // console.log(value);
    let fields =
        "(customer_id,app_id,device_type,dev_mod,report_flag,total_collection,adv_pay,adv_value,grace_period_flag,grace_value,redirection_flag,gst_flag,pay_mode_flag,created_at)",
