...
const rate_dtls_list_monthly = async (req, res) => {
  try {
    const schema = Joi.object({
      dev_mod: Joi.string().valid("D", "F").required(),
    ...
    var data = await db_Select(
      "sl_no,seller_id,customer_id,vehicle_id,no_of_days,vehicle_rate",
...
const gst_list = async (req, res) => {
  try {
    ...
    let where = `customer_id=${userData.customer_id} AND gst_flag='Y'`;
    var data = await db_Select(
      "gst_mode,gst_number,cgst,sgst,igst",
      "md_gst",
      where,
      null,
    );
