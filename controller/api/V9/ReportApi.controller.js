...
const vehicle_wise = async (req, res) => {
  try {
    ...
    const from_date = convertUTCToIST(value.from_date);
    const to_date = convertUTCToIST(value.to_date);
    var select = `d.vehicle_name vehicleType, COUNT(b.receipt_no) tot_vehi, SUM(c.paid_amt) tot_amt, SUM(c.advance_amt) advance_amt, c.pay_mode`,
      ...
      whr = `a.receipt_no=b.receipt_no AND a.receipt_no=c.receipt_no AND a.vehicle_id=d.vehicle_id AND a.car_out_flag = 'Y' AND a.intype = '${value.intype}' AND b.date_time_out BETWEEN '${from_date}' AND '${to_date}' AND a.customer_id = '${userData.customer_id}' AND b.user_id = '${value.customerUserName}'`,
      order = "GROUP BY a.vehicle_id, d.vehicle_name,c.pay_mode";
    var res_dt = await db_Select(select, table_name, whr, order);

    // ✅ convert inside msg
    if (res_dt && res_dt.msg) {
      res_dt.msg = res_dt.msg.map((row) => ({
        ...row,
        tot_amt: Number(row.tot_amt),
        advance_amt: Number(row.advance_amt),
      }));
    }
...
const unbilled = async (req, res) => {
  try {
    ...
    var select = `a.receipt_no, a.date_time_in, a.device_id, d.vehicle_name, a.vehicle_no, f.operator_name, g.advance_amt`,
      ...
      whr = `a.car_out_flag = 'N' AND a.intype = '${value.intype}' AND a.date_time_in BETWEEN '${from_date}' AND '${to_date}' AND a.customer_id = '${userData.customer_id}'`,
...
const dashboard = async (req, res) => {
  try {
    ...
    var select = `IFNULL(SUM(a.paid_amt),0) as paid_amt, a.pay_mode`,
      // table_name = 'td_vehicle_in a, td_receipt b',
      table_name = "td_receipt a, td_vehicle_out b",
      // whr = `a.receipt_no=b.receipt_no AND DATE(a.date_time_in)='${datetime}' AND a.customer_id = '${userData.customer_id}' AND a.user_id_in = '${value.customerUserName}'`
      whr = `a.receipt_no=b.receipt_no AND DATE(b.date_time_out)='${datetime}' AND b.user_id = '${value.customerUserName}'`;
    order = `GROUP BY a.pay_mode`;
    var paid_amt = await db_Select(select, table_name, whr, order);

    var select2 = `count(*) as vehicle_out`,
      table_name2 = "td_vehicle_out a, td_vehicle_in b",
      whr2 = `a.receipt_no=b.receipt_no AND DATE(a.date_time_out)='${datetime}' AND b.customer_id = '${userData.customer_id}' AND a.user_id = '${value.customerUserName}'`;
    var vehicle_out = await db_Select(select2, table_name2, whr2, null);

    var select3 = `count(*) as vehicle_in`,
      table_name3 = "td_vehicle_in a",
      whr3 = `DATE(a.date_time_in)='${datetime}' AND a.customer_id = '${userData.customer_id}' AND a.user_id_in = '${value.customerUserName}'`;
...
const detail_report = async (req, res) => {
  try {
    ...
    const from_date = convertUTCToIST(value.from_date);
    const to_date = convertUTCToIST(value.to_date);
    var select = `a.receipt_no, a.date_time_in, a.device_id, d.vehicle_name, a.vehicle_no, b.date_time_out, b.device_id device_id_out, c.base_amt, c.cgst, c.sgst, c.igst, c.paid_amt, c.advance_amt, c.pay_mode, f.operator_name`,
      table_name =
        "td_vehicle_in a, td_vehicle_out b, td_receipt c, md_vehicle d, md_user e, md_operator f",
      whr = `a.receipt_no=b.receipt_no AND a.receipt_no=c.receipt_no AND a.vehicle_id=d.vehicle_id AND a.user_id_in=e.id AND e.user_id=f.user_id AND a.car_out_flag = 'Y' AND a.intype = '${value.intype}' AND b.date_time_out BETWEEN '${from_date}' AND '${to_date}' AND a.customer_id = '${userData.customer_id}' AND b.user_id = '${value.customerUserName}'`,
      order = "ORDER BY a.receipt_no";
...
const shift_wise = async (req, res) => {
  try {
    ...
    const userData = req.user;
    const select = `a.shift_name, (SELECT COUNT(*) FROM td_vehicle_in AS b, td_receipt AS e WHERE e.sl_no = b.vehicle_in_id AND date (b.date_time_in) >= '${value.from_date}' AND date(b.date_time_in) <= '${value.to_date}' AND time(b.date_time_in) >= a.f_time AND time(b.date_time_in) <= a.t_time) AS quantity, (SELECT SUM(f.paid_amt) FROM td_vehicle_in AS c, td_receipt AS f WHERE f.sl_no = c.vehicle_in_id AND f.trans_flag = 'P' AND date(c.date_time_in) >= '${value.from_date}' AND date(c.date_time_in) <= '${value.to_date}' AND time(c.date_time_in) >= a.f_time AND time(c.date_time_in) <= a.t_time) AS totalAmount, (SELECT SUM(g.advance_amt) FROM td_vehicle_in AS d, td_receipt AS g WHERE g.sl_no = d.vehicle_in_id AND g.trans_flag = 'A' AND date(d.date_time_in) >= '${value.from_date}' AND date(d.date_time_in) <= '${value.to_date}' AND time(d.date_time_in) >= a.f_time AND time(d.date_time_in) <= a.t_time) AS TotalAdvance , h.operator_name,h.user_id`,
      tablename = `md_shift AS a, md_operator AS h, td_receipt AS i`,
      where = `h.customer_id=a.customer_id AND h.customer_id='${userData.customer_id}' AND i.user_id = '${value.customerUserName}'`;
...
const operator_wise = async (req, res) => {
  try {
    ...
    const from_date = convertUTCToIST(value.from_date);
    const to_date = convertUTCToIST(value.to_date);
    var select = `a.device_id mc_srl_no_out,e.operator_name opratorName,g.vehicle_name vehicleType,COUNT(a.receipt_no) tot_vehi,SUM(b.paid_amt) tot_amt,SUM(b.advance_amt) AS adv_amt`,
      table_name =
        "td_vehicle_out a,td_receipt b,md_user d,md_operator e,td_vehicle_in f,md_vehicle g",
      whr = `a.receipt_no = b.receipt_no
                   AND a.user_id    = d.id
                   AND d.user_id    = e.user_id
                   AND a.receipt_no = f.receipt_no
                   AND f.vehicle_id = g.vehicle_id
                   AND f.intype = '${value.intype}'
                   AND a.date_time_out BETWEEN '${from_date}' AND '${to_date}' 
                   AND a.user_id = '${value.customerUserName}'`,
...
const shift_wise_report = async (req, res) => {
  try {
    ...
    console.log(shift_time.msg[0], "shift_time.msg[0]");
    // console.log(whr, "------------shift where");
    var select = `b.device_id mc_srl_no_out, d.vehicle_name vehicleType, COUNT(b.receipt_no) tot_vehi, SUM(c.paid_amt) tot_amt, SUM(c.advance_amt) advance_amt, c.pay_mode, f.operator_name opratorName`,
      table_name =
        "td_vehicle_in a, td_vehicle_out b, td_receipt c, md_vehicle d, md_user e, md_operator f",
      whr = `a.receipt_no=b.receipt_no AND a.receipt_no=c.receipt_no AND a.vehicle_id=d.vehicle_id AND a.user_id_in=e.id AND e.user_id=f.user_id AND a.car_out_flag = 'Y' AND a.intype = '${data.intype}' AND DATE(b.date_time_out) BETWEEN '${data.frm_dt}' AND '${data.to_dt}' AND TIME(b.date_time_out) BETWEEN '${ftime}' AND '${ttime}' AND a.customer_id = '${custId}'`,
      order =
        "GROUP BY a.user_id_in,b.device_id,d.vehicle_name,c.pay_mode, f.operator_name";
...
const monthly_subscription_report = async (req, res) => {
  try {
    var custId = req.user.customer_id;
    var user_id_in = req.user.id;
    var data = req.body;
    var select =
      "receipt_no,vehicle_no,start_date,end_date,paid_amt,cgst,sgst,igst";
    var table_name = "td_vehicle_subscription";
    var whr = `DATE(created_at) >= '${data.from_date}' AND DATE(created_at) <= '${data.to_date}' AND status = 'A' AND customer_id = '${custId}' AND user_id_in = '${user_id_in}'`;
...
const monthly_subsc_expiry_report = async (req, res) => {
  try {
    const schema = Joi.object({
    ...
    var user_id_in = req.user.id;
    var data = req.body;
    var select = "receipt_no,vehicle_no,start_date,end_date,paid_amt";
    var table_name = "td_vehicle_subscription";

    if (value.exp_type == "1") {
      var whr = `end_date BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 30 DAY) AND customer_id = '${custId}' AND user_id_in = '${user_id_in}'`;
    } else {
      var whr = `end_date >= DATE_SUB(CURDATE(), INTERVAL 30 DAY) AND customer_id = '${custId}' AND user_id_in = '${user_id_in}' AND STATUS ='E' `;
    }

    var order = "order by end_date ASC";
    var res_dt = await db_Select(select, table_name, whr, order);
