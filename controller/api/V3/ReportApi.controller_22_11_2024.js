...
const vehicle_wise = async (req, res) => {
    try {
        ...
        //     whr = `a.receipt_no=b.receipt_no AND a.receipt_no=c.receipt_no AND a.vehicle_id=d.vehicle_id AND a.car_out_flag = 'Y' AND DATE(b.date_time_out) BETWEEN '${value.from_date}' AND '${value.to_date}' AND a.customer_id = '${userData.customer_id}' AND (a.user_id_in = '${value.customerUserName}' OR b.user_id = '${value.customerUserName}')`,
        //     order = 'GROUP BY a.vehicle_id, d.vehicle_name';

       var sql =  `select vehicleType,sum(tot_vehi)tot_vehi,sum(tot_amt)tot_amt,sum(advance_amt)advance_amt
        from (
        select d.vehicle_name vehicleType, 
                COUNT(b.receipt_no) tot_vehi, 
                SUM(c.paid_amt) tot_amt, 
                0  advance_amt
        from   td_vehicle_in a, td_vehicle_out b, td_receipt c, md_vehicle d
        where  a.receipt_no=b.receipt_no 
        AND    b.receipt_no=c.receipt_no 
        AND    a.vehicle_id=d.vehicle_id 
        AND    a.car_out_flag = 'Y' 
        AND DATE(b.date_time_out) BETWEEN '${value.from_date}' AND '${value.to_date}'
        and    b.user_id = ${value.customerUserName}
        AND a.customer_id = ${userData.customer_id}
        GROUP BY a.vehicle_id, d.vehicle_name
        UNION
        select d.vehicle_name vehicleType, 
                COUNT(b.receipt_no) tot_vehi, 
                0 tot_amt, 
                SUM(c.advance_amt)advance_amt
        from   td_vehicle_in a, td_vehicle_out b, td_receipt c, md_vehicle d
        where  a.receipt_no=b.receipt_no 
        AND    b.receipt_no=c.receipt_no 
        AND    a.vehicle_id=d.vehicle_id 
        AND    a.car_out_flag = 'Y' 
        AND DATE(b.date_time_out) BETWEEN '${value.from_date}' AND '${value.to_date}'
        and    c.user_id = '${value.customerUserName}'
        AND a.customer_id = '${userData.customer_id}'
        GROUP BY a.vehicle_id, d.vehicle_name)a
        group by vehicleType`
        var data = await db_Select(null, null, null, null, true, sql)
...
const dashboard = async (req, res) => {
    ...
    //   paid_amt = await db_Select(select, table_name, whr, null);
      // }else{
      //     var select = `0 paid_amt`,
      //         table_name = 'td_vehicle_in a',
      //         whr = `DATE(a.date_time_in)='${datetime}' AND a.customer_id = '${userData.customer_id}' AND a.user_id_in = '${value.customerUserName}'`
      //     paid_amt = await db_Select(select, table_name, whr, null)
      // }

      var select = `ifnull(SUM(b.paid_amt),0) as paid_amt`,
          table_name = "td_vehicle_in a, td_receipt b,td_vehicle_out c",
          whr = `a.receipt_no=b.receipt_no AND b.receipt_no=c.receipt_no AND DATE(a.date_time_in)='${datetime}' AND a.customer_id = '${userData.customer_id}' AND c.user_id = '${value.customerUserName}'`;
        paid_amt = await db_Select(select, table_name, whr, null);
  
      var select2 = `count(*) as vehicle_out `,
        table_name2 = "td_vehicle_in a, td_vehicle_out b ",
        whr2 = `a.receipt_no=b.receipt_no AND DATE(b.date_time_out)='${datetime}' AND a.customer_id = '${userData.customer_id}'  AND b.user_id = '${value.customerUserName}'`;
      var vehicle_out = await db_Select(select2, table_name2, whr2, null);
  
      var select3 = `count(*) as vehicle_in `,
        table_name3 = "td_vehicle_in a",
        whr3 = `DATE(date_time_in)='${datetime}' AND a.customer_id = '${userData.customer_id}' AND a.user_id_in = '${value.customerUserName}'`;
      var vehicle_in = await db_Select(select3, table_name3, whr3, null);

      var select4 = `ifnull(SUM(b.advance_amt),0) as advance_amt`,
      table_name4 = "td_vehicle_in a, td_receipt b",
      whr4 = `a.receipt_no=b.receipt_no AND DATE(a.date_time_in)='${datetime}' AND a.customer_id = '${userData.customer_id}' AND b.user_id = '${value.customerUserName}'`;
    var advance_amt = await db_Select(select4, table_name4, whr4, null);
...
const detail_report = async (req, res) => {
    try {
        ...
        //     whr = `a.receipt_no=b.receipt_no AND a.receipt_no=c.receipt_no AND a.vehicle_id=d.vehicle_id AND a.user_id_in=e.id AND e.user_id=f.user_id AND a.car_out_flag = 'Y' AND DATE(b.date_time_out) BETWEEN '${value.from_date}' AND '${value.to_date}' AND a.customer_id = '${userData.customer_id}' AND (a.user_id_in = '${value.customerUserName}' OR b.user_id = '${value.customerUserName}')`,
        //     order = 'ORDER BY a.receipt_no';

        var sql = `select receipt_no,date_time_in,vehicle_no,sum(advance_amt)advance_amt,sum(paid_amt)paid_amt
        from (
        select a.receipt_no receipt_no, 
               a.date_time_in date_time_in, 
               a.vehicle_no vehicle_no, 
               0 advance_amt ,
               c.paid_amt paid_amt
        from   td_vehicle_in a, td_vehicle_out b, td_receipt c
        where  a.receipt_no=b.receipt_no 
        ...
        AND DATE(b.date_time_out) BETWEEN '${value.from_date}' AND '${value.to_date}'
        AND a.customer_id = '${userData.customer_id}' 
        AND c.user_id = '${value.customerUserName}' )a
        GROUP by receipt_no,date_time_in,vehicle_no`
        var data = await db_Select(null, null, null, null, true, sql)
...
const shift_wise = async (req, res) => {
    try {
        ...
        const select = `a.shift_name, (SELECT COUNT(*) FROM td_vehicle_in AS b, td_receipt AS e WHERE e.vehicle_in_id = b.vehicle_in_id AND date (b.date_time_in) >= '${value.from_date}' AND date(b.date_time_in) <= '${value.to_date}' AND time(b.date_time_in) >= a.f_time AND time(b.date_time_in) <= a.t_time) AS quantity, (SELECT SUM(f.paid_amt) FROM td_vehicle_in AS c, td_receipt AS f WHERE f.vehicle_in_id = c.vehicle_in_id AND f.trans_flag = 'P' AND date(c.date_time_in) >= '${value.from_date}' AND date(c.date_time_in) <= '${value.to_date}' AND time(c.date_time_in) >= a.f_time AND time(c.date_time_in) <= a.t_time) AS totalAmount, (SELECT SUM(g.advance_amt) FROM td_vehicle_in AS d, td_receipt AS g WHERE g.vehicle_in_id = d.vehicle_in_id AND g.trans_flag = 'A' AND date(d.date_time_in) >= '${value.from_date}' AND date(d.date_time_in) <= '${value.to_date}' AND time(d.date_time_in) >= a.f_time AND time(d.date_time_in) <= a.t_time) AS TotalAdvance , h.operator_name,h.user_id`,
            ...
            where = `h.customer_id=a.customer_id AND h.customer_id='${userData.customer_id}' AND a.user_id_in = '${value.customerUserName}'`;
        ...
        // AND    a.car_out_flag = 'Y' 
        // AND    DATE(b.date_time_out) BETWEEN '${value.from_date}' AND '${value.to_date}'
        // and    TIME(b.date_time_out) >= (select f_time FROM md_shift where customer_id = '${userData.     customer_id}' and shift_id = '${value.shift_id}')
        // and    TIME(b.date_time_out) <= (select t_time FROM md_shift where customer_id = '${userData.customer_id}' and shift_id = '${value.shift_id}' )   
        // and    b.user_id = '${value.customerUserName}'
        // AND a.customer_id = '${userData.customer_id}'
        // GROUP BY a.vehicle_id, d.vehicle_name
        // UNION
...
const operator_wise = async (req, res) => {
    try {
        ...
        var sql = `select opratorName,vehicle_name,sum(tot_vehi)tot_vehi,sum(tot_amt)tot_amt,SUM(advance_amt)advance_amt
        from (
        select f.operator_name opratorName,
               d.vehicle_name vehicle_name,
               COUNT(b.receipt_no) tot_vehi, 
               SUM(c.paid_amt) tot_amt,
               0 advance_amt
        from   td_vehicle_in a, td_vehicle_out b, td_receipt c,md_vehicle d,md_user e,md_operator f
        where  a.receipt_no=b.receipt_no 
        AND    b.receipt_no=c.receipt_no 
        and    a.vehicle_id = d.vehicle_id
        and    b.user_id = e.id
        ...
        and    b.user_id = '${value.customerUserName}'
        AND    a.customer_id = '${userData.customer_id}'
        group by f.operator_name,d.vehicle_name
        UNION
        select f.operator_name opratorName,
               d.vehicle_name vehicle_name,
               COUNT(b.receipt_no) tot_vehi, 
        ...
        and    c.user_id = '${value.customerUserName}'
        AND    a.customer_id = '${userData.customer_id}'
        group by f.operator_name,d.vehicle_name)a
        group by opratorName,vehicle_name`
        var data = await db_Select(null, null, null, null, true, sql)
