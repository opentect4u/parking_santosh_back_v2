...
redirection_flag='${value.redirect_flag == "Y" ? "Y" : "N"
        ...
        : "(app_id,customer_id,device_type,dev_mod,report_flag,total_collection,adv_pay,adv_value,grace_period_flag,grace_value,redirection_flag,gst_flag,pay_mode_flag,qr_code_flag,day_wise_rate,created_by,created_at)",
      values = `('${value.app_id}','${value.cust_id}','M','${value.dev_mode
        ...
        }','${value.gst_flag == "Y" ? "Y" : "N"}','${value.pay_mode_flag && value.pay_mode_flag == "Y" ? "Y" : "N"
    ...
    if (oldData) {
      ...
      if (oldData.gst_flag !== (value.gst_flag == "Y" ? "Y" : "N"))
        changes.push(`gst_flag: '${oldData.gst_flag}' → '${value.gst_flag == "Y" ? "Y" : "N"}'`);
      ...
      if (oldData.day_wise_rate !== (value.day_wise_rate == "Y" ? "Y" : "N"))
        changes.push(`day_wise_rate: '${oldData.day_wise_rate}' → '${value.day_wise_rate == "Y" ? "Y" : "N"}'`);
      ...
      let users = await db_Select("user_id,device_id,fcm_token", "md_user", `customer_id='${value.cust_id}' AND allow_flag='Y' AND user_type = 'O'`, null);
        ...
        for (let user of users.msg) {
          if (user.fcm_token) {
            ...
            const message = {
              ...
              notification: { title: "force_logout", body: "Your session expired due to settings update." },
              data: { title: "force_logout", body: "Your session expired due to settings update." }
            };
            try {
