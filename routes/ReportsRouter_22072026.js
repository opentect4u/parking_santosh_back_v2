...
reportRouter.post("/get_devices_by_location", AuthCheckedMW, async (req, res) => {
  try {
    const { custId } = req.body;
    const devices = await db_Select("*", "md_setting", `customer_id='${custId}'`);
    res.send(devices);
  } catch (err) {
    console.error(err);
    res.send({ suc: 0, msg: [] });
...
reportRouter.post("/get_vehicles_by_location", AuthCheckedMW, async (req, res) => {
  try {
    const { custId } = req.body;
    const vehicles = await db_Select("vehicle_id, vehicle_name", "md_vehicle", `customer_id='${custId}'`);
    res.send(vehicles);
  } catch (err) {
    console.error(err);
    res.send({ suc: 0, msg: [] });
  ...
  async (req, res) => {
    ...
    // MAIN SELECT QUERY (WITH PAGINATION)
      let select = `
        ...
        c.cgst, c.sgst, c.igst, c.paid_amt, c.pay_mode, f.operator_name
      `;
...
reportRouter.post("/get_unbilled_report", AuthCheckedMW, async (req, res) => {
  try {
    ...
    let baseWhere = `a.car_out_flag = 'N' AND a.date_time_in BETWEEN '${data.frm_dt}' AND '${data.to_dt}' AND a.customer_id = '${data.custId}'`;
    ...
    const totalRec = await db_Select("COUNT(*) as count", table_name, baseWhere, null);
    let totalRecords = totalRec.suc > 0 ? totalRec.msg[0].count : 0;
    ...
    const grandTotalsQuery = await db_Select(`SUM(g.advance_amt) AS advance_amt`, table_name, baseWhere, null);
    const grandTotals = grandTotalsQuery.suc > 0 ? grandTotalsQuery.msg[0] : { advance_amt: 0 };
  ...
  async (req, res) => {
    try {
      ...
      var select = `b.device_id mc_srl_no_out, d.vehicle_name vehicleType, COUNT(b.receipt_no) tot_vehi, SUM(c.paid_amt) paid_amt, SUM(c.advance_amt) as advance_amt, SUM(c.base_amt) base_amt, SUM(c.cgst) cgst, SUM(c.sgst) sgst, SUM(c.igst) igst, f.operator_name opratorName`,
        table_name = "td_vehicle_in a, td_vehicle_out b, td_receipt c, md_vehicle d, md_user e, md_operator f",
        whr = `a.receipt_no=b.receipt_no AND a.receipt_no=c.receipt_no AND a.vehicle_id=d.vehicle_id AND a.user_id_in=e.id AND e.user_id=f.user_id AND a.car_out_flag = 'Y' AND b.date_time_out BETWEEN '${data.frm_dt}' AND '${data.to_dt}' AND a.customer_id = '${data.custId}'`,
        order = "GROUP BY a.user_id_in, b.device_id, d.vehicle_name ORDER BY a.user_id_in, b.device_id, d.vehicle_name";
...
reportRouter.post("/get_combine_repo_new", AuthCheckedMW, async (req, res) => {
  try {
    var custId = req.session.user.userData ? req.session.user.userData.customer_id : (req.session.user.user_data ? req.session.user.user_data.customer_id : 0);
    var data = req.body;
    const draw = parseInt(data.draw) || 1;
    const start = parseInt(data.start) || 0;
    const length = parseInt(data.length) || 50;
    ...
    var select = `f.operator_name, a.device_id, b.vehicle_name vehicleType, SUM(c.advance_amt) advance_amt, SUM(c.paid_amt) paid_amt, SUM(c.base_amt) base_amt, SUM(c.cgst) cgst, SUM(c.sgst) sgst, SUM(c.igst) igst`,
      ...
      whr = `a.vehicle_id = b.vehicle_id AND a.receipt_no = c.receipt_no AND a.receipt_no = d.receipt_no AND c.user_id = e.id AND e.user_id = f.user_id AND a.customer_id = '${data.custId}' AND a.vehicle_id = '${data.vehicle_id}' AND d.date_time_out BETWEEN '${data.frm_dt}' AND '${data.to_dt}'`;
  ...
  reportRouter.post("/get_combine_repo_dev_new", AuthCheckedMW, async (req, res) => {
    try {
      ...
      var select = `f.operator_name, d.device_id, b.vehicle_name vehicleType, SUM(c.advance_amt) advance_amt, SUM(c.paid_amt) paid_amt, SUM(c.base_amt) base_amt, SUM(c.cgst) cgst, SUM(c.sgst) sgst, SUM(c.igst) igst`,
        table_name = "td_vehicle_in a, md_vehicle b, td_receipt c, td_vehicle_out d, md_user e, md_operator f",
        whr = `a.vehicle_id = b.vehicle_id AND a.receipt_no = c.receipt_no AND a.receipt_no = d.receipt_no AND c.user_id = e.id AND e.user_id = f.user_id AND a.customer_id = '${data.custId}' AND d.device_id = '${data.device_id}' AND d.date_time_out BETWEEN '${data.frm_dt}' AND '${data.to_dt}'`;
...
reportRouter.post("/get_user_wise_report_new", AuthCheckedMW, async (req, res) => {
  try {
    var custId = req.session.user.user_data ? req.session.user.user_data.customer_id : (req.session.user.userData ? req.session.user.userData.customer_id : 0);
    var data = req.body;
    const draw = parseInt(data.draw) || 1;
    const start = parseInt(data.start) || 0;
    const length = parseInt(data.length) || 50;
    ...
    var select = `b.device_id mc_srl_no_out, COUNT(b.receipt_no) tot_vehi, SUM(c.paid_amt) paid_amt, SUM(c.advance_amt) advance_amt, SUM(c.base_amt) base_amt, SUM(c.cgst) cgst, SUM(c.sgst) sgst, SUM(c.igst) igst, f.operator_name opratorName`,
      table_name = "td_vehicle_in a, td_vehicle_out b, td_receipt c, md_user e, md_operator f",
  ...
  async (req, res) => {
    try {
      ...
      var select = `b.device_id mc_srl_no_out, d.vehicle_name vehicleType, COUNT(b.receipt_no) tot_vehi, SUM(c.paid_amt) paid_amt, SUM(c.advance_amt) AS advance_amt, SUM(c.base_amt) base_amt, SUM(c.cgst) cgst, SUM(c.sgst) sgst, SUM(c.igst) igst, f.operator_name opratorName, g.shift_name, g.f_time, g.t_time`,
        table_name = "td_vehicle_in a, td_vehicle_out b, td_receipt c, md_vehicle d, md_user e, md_operator f, md_shift g",
      ...
      let groupBy = "GROUP BY a.user_id_in, b.device_id, d.vehicle_name, f.operator_name, g.shift_name, g.f_time, g.t_time";
  ...
  async (req, res) => {
    try {
      ...
      var select = `b.device_id mc_srl_no_out, d.vehicle_name vehicleType, COUNT(b.receipt_no) tot_vehi, SUM(c.paid_amt) paid_amt, SUM(c.advance_amt) AS advance_amt, SUM(c.base_amt) base_amt, SUM(c.cgst) cgst, SUM(c.sgst) sgst, SUM(c.igst) igst, f.operator_name opratorName, g.shift_name, g.f_time, g.t_time`,
        table_name = "td_vehicle_in a, td_vehicle_out b, td_receipt c, md_vehicle d, md_user e, md_operator f, md_shift g",
        whr = `a.receipt_no=b.receipt_no AND a.receipt_no=c.receipt_no AND a.vehicle_id=d.vehicle_id AND a.user_id_in=e.id AND e.user_id=f.user_id AND a.customer_id = g.customer_id AND TIME(b.date_time_out) BETWEEN g.f_time AND g.t_time AND a.car_out_flag = 'Y' AND b.date_time_out BETWEEN '${data.frm_dt}' AND '${data.to_dt}' AND a.customer_id = '${data.custId}'`,
        order = "GROUP BY a.user_id_in, b.device_id, d.vehicle_name, f.operator_name, g.shift_name, g.f_time, g.t_time ORDER BY g.f_time";
