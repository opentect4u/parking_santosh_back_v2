...
const { register, login, test, change_password, check_report_password, logout } = require('../../controller/api/V8/Auth.controller');
const { checkedToken } = require('../../middleware/ApiAuthChecked.middleware');
const { vehicle_list } = require('../../controller/api/V8/Vehicle.controller');
const { general_settings, receipt_setting, rate_dtls_list, gst_list, fixed_rate_dtls_list, my_shift, fixed_rate_dtls_list_old } = require('../../controller/api/V8/Master.controller');
const { car_in, search_car, out_pass, car_in_fixed, car_advance_amount, car_in_out, car_out_pass } = require('../../controller/api/V8/CarInCarOut.controller');
const { vehicle_wise, detail_report, shift_wise, operator_wise, unbilled, dashboard, shift_wise_report } = require('../../controller/api/V8/ReportApi.controller');
const { app_update } = require('../../controller/api/V8/AppUpdate');
...
Api.post('/master/fixed_rate_dtls_list_old',checkedToken, fixed_rate_dtls_list_old); // for vpark
...
Api.post('/car/search_car',checkedToken, search_car); // change only for vpark

Api.post('/car/car_advance_amount',checkedToken, car_advance_amount);


Api.post('/car/out_pass',checkedToken, out_pass);
Api.post('/car/car_out_pass',checkedToken, car_out_pass); // for v park
